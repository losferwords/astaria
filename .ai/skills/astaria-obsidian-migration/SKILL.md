...
For richer background and the preserved WorldAnvil export investigation, read `.ai/context/astaria-project.md`. For cultural references, correct demonyms/forms of address, and FATE Core generation rules, read `.ai/context/astaria-meta-and-fate.md`.
...
- Treat `Энциклопедия/Секреты/` as private GM-only context. Never transfer spoilers or hidden knowledge from secret notes into public encyclopedia articles, player-facing summaries, or published Quartz pages unless the user explicitly asks to reveal a specific secret.
- Store canonical saga prose as literature under `Энциклопедия/Литература/`.
...
- Exclude `Идеи/` from lore/source research by default. It is a private idea workbench, not canon. Use it only when the user explicitly asks to use or canonize a specific idea.
- When migrating WorldAnvil text, correct obvious typos, punctuation issues, and rough rushed phrasing lightly. Do not change facts, continuity, or the user's intent.
...
Astaria's current Quartz setup lives in `_quartz/`. Do not edit `_quartz/content/` as source; it is generated by `_scripts/sync_quartz_content.rb`. The script copies only `publish: true` notes from `Энциклопедия/`, `Хронология/`, and `Карты/`, then copies only referenced local assets. Keep `.ai/`, `Идеи/`, `World-Астария-2fa/`, templates, raw imports, GM-only notes, and FATE mechanics private unless the user explicitly asks to publish them.

Public URLs are English and independent from Russian vault paths. The sync script places generated pages into English category paths such as `gods/mercate.md`, uses `/map` and `/timeline` for the main map and chronology, and preserves Russian titles as aliases so Obsidian-style links still resolve. Prefer an explicit `public_slug` frontmatter value when the automatic slug derived from local image names is not suitable.
